...
| Middleware/引擎 | 请求处理精细度 | ✅ 实现: pi-embedded-runner自有引擎 | ✅ 实现: 13层Middleware管道 | ✅ 实现: 18层Middleware(已知最多) | ✅ 实现: 自有Agent loop | ❓ 闭源 | ✅ 实现: QueryEngine六层统一内核 | ✅ 实现: agent-graph-store图状态机 | ✅ 实现: Effect-TS v4 fiber架构，结构化并发 |
...
| 威胁检测 | 运行时检测恶意行为 | - | - | - | ✅ 实现: Tirith威胁检测引擎。效果: 异常命令序列检测+告警，PII脱敏 | ❓ 闭源 | ✅ 实现: SandboxDoctorSection诊断。效果: 沙箱健康检查+异常诊断 | ❓ | - |
| Vault/Secret | 凭证加密存储和轮换 | ⚠️ 实现: secret-ref配置文件引用。效果: 非独立Vault，引用间接管理 | - | ⚠️ 实现: `.env`+`config.yaml`明文+`credential_loader.py`(从`~/.claude/.credentials.json`自动加载)。效果: 明文存储，无加密层。无Vault/Keyring/HSM | ⚠️ 实现: 5种凭证源(`credential_sources.py`): env/auth.json/OAuth/config/gcloud。全部明文存储(`.env`+`auth.json`)。效果: 30+种secret prefix日志脱敏(`redact.py`)，3个RotatingFileHandler+RedactingFormatter。**无任何加密存储**，仅文件系统0600权限 | ❓ 闭源 | ❓ 推断: Anthropic API key本地加密 | ❓ | ✅ 实现: identity package。效果: 身份认证模块，非通用Vault |
...
| 多租户隔离 | 租户级资源配额+数据隔离 | - | - | ⚠️ 实现: user_id级隔离+K3s Pod部署模式。效果: 用户级数据分离，最接近多租户雏形。缺失: 租户级资源配额、配置分离 | - | ❓ 闭源 | - | - | - |
...
| Memory 系统 | 跨会话记住用户偏好/事实 | ✅ 实现: AGENTS.md/SOUL.md文件注入+sqlite-vec向量数据库。配置: workspace根目录放AGENTS.md/SOUL.md。效果: 声明式记忆+本地语义搜索 | ✅ 实现: AGENTS.md文件解析。配置: 项目根目录放AGENTS.md。效果: 项目级上下文自动注入 | ✅ 实现: MemoryMiddleware(MW栈第12层)。配置: `memory.enabled=true`。效果: 自动记忆管理，旧消息flush到memory queue | ✅ 实现: Honcho/mem0双后端+Mem0 user_id filter。配置: `config.yaml`→`memory.provider`(honcho/mem0)。效果: 自适应记忆+多用户隔离，跨会话保持 | - | ✅ 实现: memdir+SessionMemory分层记忆(5层)。配置: `.claude/memdir/`+`CLAUDE.md`。效果: 已知产品最完整分层记忆体系: sessionStorage→memdir→SessionMemory→compact→hooks | ❌ 确认无跨session记忆。仅有agents_md.rs项目级上下文+agent-graph-store拓扑持久化 | - ⟳ 源码确认无跨会话记忆，仅会话内compaction(SUMMARY_TEMPLATE含Constraints/Preferences) |
| Vector Memory | 语义搜索历史对话 | ✅ 实现: sqlite-vec本地向量数据库。效果: 本地语义检索，不依赖外部向量DB | - | - | ✅ 实现: memory tool(LLM驱动语义搜索)。效果: 基于内容的记忆检索，非向量索引 | - | ❓ | - | - |
| Session Search | 全文搜索所有历史会话 | - | - | ✅ 实现: `POST /api/threads/search`—metadata精确匹配+status过滤+分页。配置: `ThreadSearchRequest{metadata,status,limit,offset}`。效果: 按updated_at降序，user隔离。**无FTS全文搜索** | ✅ 实现: FTS5+LLM摘要生成(`session_search` tool)。配置: `config.yaml`→`memory.fts5`。效果: 自然语言搜索历史会话，LLM二次摘要 | ❓ 闭源 | ✅ 实现: 多层搜索: `/resume`(Fuse.js模糊匹配,tag/branch/worktree过滤)+agenticSessionSearch(AI语义排序,最多100会话x2000字符)+transcriptSearch(WeakMap缓存实时搜索)。配置: `FUSE_THRESHOLD=0.3`。效果: 模糊+语义+实时三重搜索 | ✅ 实现: session_index.jsonl(append-only反向扫描)+SQLite instr()子串匹配+TUI Session Picker(分页25条)+Ctrl+R反向搜索。配置: `CODEX_HOME/session_index.jsonl`。效果: 多层搜索但**无FTS5** | - ⟳ |
| Agent 记忆文件 | 声明式注入项目上下文 | ✅ 实现: AGENTS.md/SOUL.md自动扫描加载。效果: 自动加载项目规范和人设 | ✅ 实现: AGENTS.md解析。效果: 项目上下文自动注入 | - | ✅ 实现: AGENTS.md/SOUL.md双文件。效果: 规范+人设分离，支持TOOLS.md工具偏好 | - | ❓ 推断: system prompt替代 | ✅ 实现: agents_md.rs模块。效果: AGENTS.md自动解析注入 | ✅ 实现: SKILL.md三级发现(`.claude/skills/`+`.agents/skills/`+`{skill,skills}/`)。效果: 多目录兼容Claude Code+agents.md标准 |
| 自动压缩 | 长对话自动摘要存储 | - | ✅ 实现: SummarizationMW(MW栈第11层)。配置: trigger=('fraction',0.85), keep=('fraction',0.10), model='gpt-4o-mini'。效果: 85%阈值自动触发摘要，旧消息卸出到`/conversation_history/{thread_id}.md` | - | ✅ 实现: compressor+自动触发(50%阈值)。效果: 上下文半自动压缩 | ❓ 闭源 | ✅ 实现: compact+hooks回调。配置: autoCompactThreshold=contextWindow-13000 tokens, 可通过`CLAUDE_AUTOCOMPACT_PCT_OVERRIDE`按百分比设置。效果: 可编程压缩策略，hook回调 | ❓ | - |
| Skills 自动管理 | Agent自动创建/改进Skills | - | - | - | ✅ 实现: Curator全生命周期引擎(`curator.py:255`)。配置: `curator.enabled=true`, `interval_hours=168`(7天), `stale_after_days=30`, `archive_after_days=90`。效果: active→stale→archived三态流转+LLM review(umbrella consolidation)+tar.gz快照回滚+90+威胁模式安全扫描+pinned防护。**永远只archive不delete** | - | - | - | - |
| Memory 多用户隔离 | 不同用户间的记忆隔离 | ✅ 实现: per-agent workspace文件系统隔离。效果: 文件级隔离 | - | ✅ 实现: per-user_id路径隔离。效果: 用户级数据分离 | ✅ 实现: Mem0 user_id filter+Honcho session_key。效果: 数据库级多用户隔离 | ❓ 闭源 | - | - | - |
