...
> v2.3 变更: 批量源码挖掘(7产品×7子代理) — 大面积补全待收集特征: ClaudeCode Loop/Search, DeerFlow压缩/Search, OpenClaw流式/Voice/Plan/Skills, Codex流式/Plan/Search, OpenCode Loop, Hermes D4/D5
...
| Loop 检测 | 防止 Agent 陷入死循环 | ✅ — 实现: tools.loopDetection: detectors(3种) + circuitBreaker + postCompactionGuard。效果: 三级检测+熔断+压缩后二次检查 | ✅ — 实现: PatchToolCalls。效果: 工具调用修复即破坏循环 | ✅ — 实现: LoopDetectionMW。效果: 独立中间件检测 | ✅ — 实现: tool_loop_guardrails。效果: Guardrails 内置检测 | ❓ — 闭源不可见 | ✅ — 实现: autoCompact.ts circuit breaker (MAX=3次连续失败) + blocking_limit (contextWindow-3000)。效果: 3次失败后跳闸, 每天节省~250K API调用 | ❓ — 未确认 | ✅ — 实现: DOOM_LOOP_THRESHOLD=3 + continue_loop_on_deny。效果: 连续3次同名工具相同输入即触发, 工具拒绝可继续/停止 |
| 上下文压缩 | Token 超限时自动压缩 | ✅ — 实现: auto-compaction + midTurnPrecheck + retry + notifier。效果: 自动触发+中途检查+重试+通知 | ✅ — 实现: SummarizationMW (MW 栈第 11 层)。配置: trigger=('fraction',0.85) keep=('fraction',0.10) model='gpt-4o-mini'。效果: 85%阈值自动触发, 保留10%最近消息, 专用摘要模型 | ✅ — 实现: DeerFlowSummarizationMiddleware。配置: summarization.enabled=false(默认)。效果: 默认关闭, 启用后 token超限自动摘要, skill文件保护 | ✅ — 实现: compressor + 自动触发 (50%阈值)。效果: 半触发 | ❓ — 闭源不可见 | ✅ — 实现: compact + hooks 回调。效果: 可编程压缩策略 | ✅ — 实现: compact_remote.rs (354行) + compact_remote_v2.rs。效果: 远程执行压缩，v1+v2 双策略 | ✅ — 实现: session/compaction.ts (652行) — isOverflow检测 + SessionProcessor。效果: Token超限自动压缩, 独立压缩事件总线 |
